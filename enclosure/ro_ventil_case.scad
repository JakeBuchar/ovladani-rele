/*******************************************************************************
 * RO VENTIL CONTROLLER ENCLOSURE
 * Project: ovladani_rele (ESP32-S3 + relay + LM2596 buck)
 * Based on: ultimatewaterproofcase_v74.scad by pbtec (CC BY-NC-SA)
 *
 * HARDWARE (inside box):
 *   ESP32-S3 DevKit       ~52 x 28 x 10 mm (2x USB-C)
 *   Relay SRD-05VDC-SL-C  ~50 x 26 x 18 mm
 *   LM2596 buck           ~43 x 21 x 14 mm
 *   DC barrel jack napajeni     -> side B (-Y), polomer 5.1 mm (prumer 10.2 mm)
 *   Rocker switch               -> viko, polomer 10 mm (prumer 20 mm)
 *   PG7 cable gland (ventil)    -> side C (-X), polomer 6.25 mm (prumer 12.5 mm)
 *
 * DIMENSIONS (Caselength/Width/Height = OUTER):
 *   Outer  125 x 80 x 46 mm
 *   Inner ~119 x 74 x 40 mm (wall-to-wall, see OpenSCAD console echo)
 *
 * CUTOUTS (3 total — origin = box center, +X right, +Y back):
 *   1. DC jack R 5.1 mm  — spodek, stena B (-Y), prumer 10.2 mm
 *   2. PG7 R 6.25 mm     — spodek, stena C (-X), prumer 12.5 mm
 *   3. Rocker R 10 mm    — VIKO (ne spodek!), prumer 20 mm
 *   Z for wall holes = vertical center of bottom half (~18.5 mm from floor)
 *
 * STANDOFFY (spodni dno, M3 moduly — upravte v Customizeru):
 *   Sroub: M3 x 6 mm mosaz nebo nylon (vrtani 2.5 mm pro samorez / 3.2 mm pilot)
 *   Sloupky: 6 mm OD, 7 mm vyska, dira 2.5 mm
 *
 *   Pohled shora (vnitrni plocha dna, stred = (0,0)):
 *
 *        +Y
 *            |
 *    -X -----+----- +X
 *   PG7      |
 *   o--rele--o  (4 diry, 44.5 x 18.46 mm)
 *            |   [ESP32 + buck volne]
 *
 *   Rele: 4 rohove diry, roztec 44.5 x 19.32 mm (stred-stred)
 *   Buck: bez standoffu (volne na dne)
 *
 * OPENSCAD PREVIEW — which Show* flags to see each hole:
 *   DC jack + PG7:  ShowBottom=true,  ShowTop=false  (F5 preview / F6 render)
 *   Rocker switch:  ShowBottom=false, ShowTop=true
 *   All three:      ShowBottom=true,  ShowTop=true  — PG7 on inner -X wall
 *                   between the two parts (rotate view if needed)
 *   Export STL spodek: ShowBottom=true,  ShowTop=false  -> DC jack + PG7
 *   Export STL viko:   ShowBottom=false, ShowTop=true   -> rocker R10
 *
 * PRINT SETTINGS:
 *   Material: PETG
 *   Walls: 3 perimeters (3 walls)
 *   Infill: 100% recommended for waterproof use (author default)
 *   Sealing: silicone cord 1.5-2 mm in groove (optional)
 *   Screws: M3 x 22-28 mm with hot-melt inserts (see console echo)
 *
 * EXPORT STL (OpenSCAD):
 *   1. Open this file in OpenSCAD
 *   2. Set $fn = 150 (Customizer or line below) for final render
 *   3. Bottom: ShowBottom=true, ShowTop=false, ShowGasket=false -> F6 Render -> F7 Export STL
 *   4. Top:    ShowBottom=false, ShowTop=true, ShowGasket=false -> F6 Render -> F7 Export STL
 *   5. Preview only: $fn=80 is fine (F5)
 ******************************************************************************/
...
// Shows the gasket
ShowGasket                = false;  // set true to preview TPU gasket separately
GasketHeight                = 1.5;
...
/* [Case settings] */
// Length of the case (OUTER) - target inner ~119 mm wall-to-wall
Caselength                = 125;
// Width of the case (OUTER) - target inner ~74 mm wall-to-wall
CaseWidth                 = 80;
// Height of the case (OUTER) - target inner ~40 mm floor-to-lid
CaseHeight                = 46;
// Split: lid ~12 mm, bottom ~34 mm for electronics
CutFromTop                = 12.0;
...
ShowSideWallHoles_A        = false;  // unused — PG7 moved to side C (-X) for visibility
...
SideWallHoleDiameter_A     = 12.5;  //[1:0.1:80]
...
// Activate customizable holes for cable gland or similar
ShowSideWallHoles_B        = false;  // cut via RoVentilBottomCutouts() in MAIN (reliable)
// Count of holes, if there is an additional screw on X or Y side the hole in the middle is not showed
CountOfSideWallHoles_B     = 1;     //[1:1:3]
// Diameter of the holes
SideWallHoleDiameter_B     = 9.0;   // typical panel mount hole for 5.5x2.1 mm jack
// Add or decrease height position (up and down, 0 = centered)
SideWallHoleOffset_Z_B     = 0;
// Add or decrease distance between the holes
SideWallHoleDistance_B     = 20;
// Add or decrease horizontal position of the holes (0 = centered)
SideWallHolePosition_B     = 0;    // centered on short wall

/* [Wall Holes settings side C (for cable gland cut)] */
// Activate customizable holes for cable gland or similar
ShowSideWallHoles_C        = false;  // cut via RoVentilBottomCutouts() in MAIN (reliable)
// Count of holes, if there is an additional screw on X or Y side the hole in the middle is not showed
CountOfSideWallHoles_C     = 1;     //[1:1:3]
// Diameter of the holes
SideWallHoleDiameter_C     = 12.5;  // PG7 panel hole (M20 thread / 12.5 mm drill)
// Add or decrease height position (up and down, 0 = centered)
SideWallHoleOffset_Z_C     = 0;
// Add or decrease distance between the holes
SideWallHoleDistance_C     = 20;
// Add or decrease horizontal position of the holes (0 = centered)
SideWallHolePosition_C     = 0;    // centered on -X long wall
...
/* [RO ventil — M3 standoffy (rele) ] */
// Zapnout montazni sloupky na spodnim dne
ShowRoVentilStandoffs       = true;
// Prumer sloupku (OD)
Standoff_OD                 = 6.0;   // [4:0.5:10]
// Vyska sloupku nad dnem
Standoff_Height             = 7.0;   // [4:0.5:12]
// Prumer diry (2.5 = M3 samorez/nylon, 3.0 = pilot pro hot-melt)
Standoff_Hole_Dia           = 2.5;   // [2.0:0.1:3.5]
// --- Rele: 4 rohove diry, roztec stred-stred (mm) ---
Relay_Hole_Spacing_X        = 44.5;  // delka
Relay_Hole_Spacing_Y        = 19.32; // sirka
RelayStandoff_Center_X      = -17;   // stred skupiny 4 der
RelayStandoff_Center_Y      = 0;

/* [RO ventil — vyrezy (POLOMER v mm, ne prumer!) ] */
// OpenSCAD cylinder() chce prumer -> v kodu je vzdy d = 2 * polomer
RockerSwitch_Radius       = 10.0;  // POLOMER diry tlacitka na viku
RockerSwitch_X            = 0;
RockerSwitch_Y            = 18;
DCJack_Radius             = 5.1;   // POLOMER diry DC barrel jack napajeni
PG7_Radius                = 6.25;  // POLOMER diry PG7 (kabel k ventilu)
PG7_Y                     = 0;
...
// RO ventil — vysledek rozmeru der (kontrola v konzoli OpenSCAD)
echo("RO ventil vyrezy: rocker R", RockerSwitch_Radius, "mm, DC jack R", DCJack_Radius, "mm, PG7 R", PG7_Radius, "mm");

// RO ventil custom cutout Z position
SideWallCutDepth          = SideWallThickness + 1.0;  // through-cut margin for CSG
BottomWallHole_Z          = BottomTopThickness + (CaseHeight - CutFromTop - BottomTopThickness) / 2;
...
translate([X_ObjectPosition,0,0]) rotate([0,0,0]) difference(){
    union(){
        BodyBottom();
        // **** RO ventil: M3 standoffy pro rele + LM2596 ****
        if (ShowBottom) RoVentilStandoffs();
    }
    // **** RO ventil: DC jack + PG7 cable gland on bottom side walls ****
    if (ShowBottom) RoVentilBottomCutouts();
}
...
translate([-X_ObjectPosition,0,Z_TopHigh+0.03]) rotate([0,Y_TopRotation,0]) difference(){
    ...
    // **** RO ventil: rocker switch hole on lid ****
    if (ShowTop) RoVentilTopCutouts();
}
...
module RoVentilBottomCutouts() {
    // Side B (-Y short wall): DC barrel jack
    translate([0, -CaseWidth/2 + SideWallThickness/2, BottomWallHole_Z])
        rotate([90, 0, 0])
        cylinder(h=SideWallCutDepth, d=2 * DCJack_Radius, center=true);
    // Side C (-X long wall): PG7 cable gland for external solenoid valve
    translate([-Caselength/2 + SideWallThickness/2, PG7_Y, BottomWallHole_Z])
        rotate([90, 0, 90])
        cylinder(h=SideWallCutDepth, d=2 * PG7_Radius, center=true);
}

module RoVentilTopCutouts() {
    // Viko ma vysku CutFromTop (0..CutFromTop) — rez musi projit celou tloustkou
    translate([RockerSwitch_X, RockerSwitch_Y, CutFromTop / 2])
        cylinder(h=CutFromTop + 0.04, d=2 * RockerSwitch_Radius, center=true);
}

module RoVentilStandoffPost() {
    translate([0, 0, BottomTopThickness - 0.01]) difference() {
        cylinder(h=Standoff_Height + 0.01, d=Standoff_OD, center=false);
        translate([0, 0, -0.01])
            cylinder(h=Standoff_Height + BottomTopThickness + 0.04, d=Standoff_Hole_Dia, center=false);
    }
}

module RoVentilStandoffs() {
    if (ShowRoVentilStandoffs) {
        color("Chocolate") {
            // Rele: 4 rohove diry, roztec Relay_Hole_Spacing_X x Y
            for (sx = [-1, 1], sy = [-1, 1])
                translate([
                    RelayStandoff_Center_X + sx * Relay_Hole_Spacing_X / 2,
                    RelayStandoff_Center_Y + sy * Relay_Hole_Spacing_Y / 2,
                    0
                ])
                    RoVentilStandoffPost();
        }
    }
}
